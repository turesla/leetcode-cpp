...
int findPeakElement(const vector<int>&num)
{
	...
	while (low<high)
	{
		...
		//compare middle element with its neighbours(if neighbours exists)
		if ((mid==0||num[mid]>num[mid-1])&&(mid==n-1||num[mid]>num[mid]))
		{
			return mid;
		}

		//if middle element is not peak and its lefet neighbor is greater than it 
