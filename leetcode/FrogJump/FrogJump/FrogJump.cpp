...
* Note:
*
* The number of stones is ¡Ý 2 and is
...
public:
	bool canCross_recursion(vector<int>&stones, int curr, int last_jump)
	{
		for (int i = curr+1; i < stones.size(); i++)
		{
			...
			if (i==stones.size()-1||canCross_recursion(stones,i,next_jump)
			{
				return true;
			}
		}
	...
	bool canCross_recursion_with_cache(vector<int>&stones, int curr, int last_jump, unordered_map<int, unordered_map<int, bool>>&cache)
	{
	...
	{
		//the 'jump ' map store the all possible 'last jumps' 
		unordered_map<int, unordered_set<int>>jumps = { {1,{0}} };
		...
		for (int i = 0; i < stones.size(); i++)
		{
			...
			for (int last_jump : jumps[i])
			{
				...
				for (int j = i+1; j < stones.size(); j++)
				{
					...
					if (stones[i]+last_jump+1<stones[j])
					{
						break;
					}
