...
vector<int>postorderTraversal2(TreeNode*root)
{
	...
	while (stack.size()>0||node!=NULL)
	{
		...
		{
			TreeNode*n = stack.back();
			//left way is finished ,keep going to the right way
...
TreeNode* createNode(int a[], int n)
{
	if (n<=0)
	{
		return NULL;
	...
	TreeNode**tree = new TreeNode*[n];// 动态生成一个指针  指向TreeNode 类型的指针数组   赋予一个二级指针tree
 ...
 void printArray(vector<int>v)
{
 ...
 int main() {
	 ...
	 TreeNode*p = createNode(a, sizeof(a) / sizeof(int));
