...
double findMedianSortedArrayHelper(int A[], int m, int B[], int n, int lowA, int highA, int lowB, int highB)
{
	...
	if (num == (m + n) / 2)
	{
		...
		//if two arrays total length is even ,then we have to find the next one ;
		int next;
...
double findMedianSortedArrays(int A[], int m, int B[], int n)
{
	...
	//if the length of array id even ,return the average of the middle two numbers; 
	if (m == 0)return n % 2 == 1 ? B[n / 2] : (B[n / 2 - 1] + B[n / 2 - 1]);
